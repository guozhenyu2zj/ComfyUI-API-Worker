...
from worker.supabase_worker import SupabaseWorker
from comfyui_api import ComfyUIClient

def process_task(task: Dict[str, Any], worker: SupabaseWorker, client: ComfyUIClient) -> bool:
    """
    处理单个任务
    ...
    try:
        if task_type == "photo_to_sample":
            photo_url = input_data["photo_url"]
            result = client.process_multiple_face_desensitization_auto(input_image=photo_url)
        elif task_type == "selfie_to_pro":
            selfie_url = input_data["selfie_url"]
            reference_photo_url = input_data["reference_photo_url"]
            result = client.process_face_swap_auto(input_image=reference_photo_url,
                                                   input_face=selfie_url)
        ...
        # 保存结果
        if not worker.save_task_result(task_id, result):
            return False
    ...
    while True:
        try:
            ...
            if task:
                ...
                process_task(task, worker, client)
            else:
                print("没有可用的任务，等待中...")
                time.sleep(interval)
